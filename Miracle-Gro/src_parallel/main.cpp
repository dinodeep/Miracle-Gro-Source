...
TODO LIST:
    ...
    3. parallelize
        - create trees in parallel - (forest-level)
            - examine speedup changes as dataset size changes?
        - creating a single tree in parallel (tree-level)
            - splitting the creation of various child nodes
            - look at speedups under different policies, OpenMP
        - fine-grained parallelism on some for loops (node-level)
            - finding the best split

    data: 
        - 20 trees, depth 5
        - compare sequential, forest, forest + tree, forest + tree + node
        - compare parallel for vs. task -- forest + tree (show that the threads were being used up)
*/
...
int main() {
    ...
    RandomForestClassifier rfc(10, 5);
    rfc.fit(m, labels);

    printf("Full Parallel Training Time: %.5fs\n", t.elapsed());
